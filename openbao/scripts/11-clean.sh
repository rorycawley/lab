...
IMAGE_NAME="openbao-demo-app:demo"
...
if [[ -e /tmp/openbao-demo-port-forward.log ]]; then
  echo "Cleanup verification failed: /tmp/openbao-demo-port-forward.log still exists" >&2
  failures=$((failures + 1))
fi
