...
APP_IMAGE="keycloak-pg-demo-app:demo"
PROXY_IMAGE="keycloak-pg-demo-proxy:demo"
PROJECT_NAME="keycloak-pg"
...
rm -rf logs
rm -f /tmp/keycloak-pg-demo-port-forward.log
...
assert_empty "Docker image $PROXY_IMAGE" "$(docker images "$PROXY_IMAGE" --format '{{.Repository}}:{{.Tag}} {{.ID}}' || true)"
if [[ -d logs ]]; then
  echo "Cleanup verification failed: logs/ still exists" >&2
  failures=$((failures + 1))
fi
if [[ -e /tmp/keycloak-pg-demo-port-forward.log ]]; then
  echo "Cleanup verification failed: /tmp/keycloak-pg-demo-port-forward.log still exists" >&2
  failures=$((failures + 1))
fi
